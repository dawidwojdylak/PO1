...
int main() {
  ...
  // zadanie domowe; lista inicjalizacyjna
  Argumenty abcd{{0, 2.5}, {1,4},{3,8}, {2,9}};
  abcd.print("abcd: ");
  Algo *s1 = new Suma();
  s1->wykonaj( abcd ).print( "Po zsumowaniu:" );

  Pierwiastek *p1 = new Pierwiastek();
  p1->wykonaj( abcd ).print( "Pierwiastki:" );
  s1->wykonaj( p1->wykonaj( abcd ) ).print( "Suma pierwiastkow:" );
  p1->wykonaj( s1->wykonaj( abcd ) ).print( "Pierwiastek sumy:" );
  delete s1;
  delete p1;
}
/* wynik
